...
import com.jackykeke.ownretromusicplayer.helper.SortOrder.PlaylistSortOrder.Companion.PLAYLIST_A_Z
import com.jackykeke.ownretromusicplayer.helper.SortOrder.PlaylistSortOrder.Companion.PLAYLIST_SONG_COUNT
import com.jackykeke.ownretromusicplayer.helper.SortOrder.PlaylistSortOrder.Companion.PLAYLIST_SONG_COUNT_DESC
import com.jackykeke.ownretromusicplayer.helper.SortOrder.PlaylistSortOrder.Companion.PLAYLIST_Z_A
...
class RealRoomRepository(
    ...
    override fun checkPlaylistExists(playListId: Long): LiveData<Boolean> =playlistDao.checkPlaylistExists(playListId)
    ...
    override suspend fun playlistWithSongs(): List<PlaylistWithSongs> =
        when(PreferenceUtil.playlistSortOrder){
            ...
            PLAYLIST_SONG_COUNT->
                playlistDao.playlistsWithSongs().sortedBy { it.songs.size }
            PLAYLIST_SONG_COUNT_DESC->
                playlistDao.playlistsWithSongs().sortedByDescending { it.songs.size }
            else -> playlistDao.playlistsWithSongs().sortedBy { it.playlistEntity.playlistName }
        }
    ...
    = songs.forEach { playlistDao.deleteSongFromPlaylist(it.playlistCreatorId,it.id) }
    ...
    =playlistDao.isSongExistsInPlaylist(songEntity.playlistCreatorId,songEntity.id)
    ...
    = if (playlistDao.playlist(favorite).isNotEmpty()){
        playlistDao.favoritesSongs(playlistDao.playlist(favorite).first().playListId)
    }else emptyList()
