 ...
 *
 * 使用 Java 代码模糊。 <p>这是高斯模糊和方框模糊之间的折衷方案它创建的模糊效果比方框模糊好得多，但比我的高斯模糊实现快 7 倍。
 * <p>我将其命名为 Stack Blur 是因为它最能描述此滤镜的内部工作原理：它在扫描图像时创建一种移动的颜色堆叠。
 * 因此它只需要在堆栈的右侧添加一个新的颜色块并删除最左边的颜色。堆栈最顶层的剩余颜色要么加一种，要么减一种，这取决于它们是在堆栈的右侧还是左侧。
 *
...
public class StackBlur {
  ...
  public static Bitmap blur(Bitmap original, float radius) {
    ...
    //在 pixels[] 中返回位图中数据的副本。每个值都是一个表示 {@link Color} 的打包 int。
    // stride 参数允许调用者允许行之间返回的像素数组中的间隙。对于正常的打包结果，
    // 只需为步幅值传递宽度。返回的颜色是 {@link ColorSpace.NamedSRGB sRGB} 颜色空间中的非预乘 ARGB 值。
  ...
  /// step 步骤1 和步骤2
  // 是因为它最能描述此滤镜的内部工作原理：它在扫描图像时创建一种移动的颜色堆叠。
  // * 因此它只需要在堆栈的右侧添加一个新的颜色块并删除最左边的颜色。
  // 堆栈最顶层的剩余颜色要么加一种，要么减一种，这取决于它们是在堆栈的右侧还是左侧。
  private static void blurIteration(
      int[] src, int w, int h, int radius, int cores, int core, int step) {
    // cores 总线程   core 线程的第几个
    int x, y, xp, yp, i;
    int sp;
    int stack_start;
    int stack_i;
...
//    0*1000/3=0;
    ...
    if (step == 1) {
      ...
      for (y = minY; y < maxY; y++) {
        ...
        for (i = 0; i <= radius; i++) {
          stack_i = i; // 0~半径
          stack[stack_i] = src[src_i]; //扫描栈
          sum_r += ((src[src_i] >>> 16) & 0xff) * (i + 1); // 取色rgb 又✖️ i
        ...
        for (i = 1; i <= radius; i++) {
          if (i <= wm) src_i += 1;
          stack_i = i + radius; // //  半径  ~ 直径
          stack[stack_i] = src[src_i];
          sum_r += ((src[src_i] >>> 16) & 0xff) * (radius + 1 - i);  // 取色rgb 又✖️ (radius + 1 - i)
