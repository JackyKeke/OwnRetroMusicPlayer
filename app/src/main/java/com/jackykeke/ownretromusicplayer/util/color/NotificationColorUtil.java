...
import static androidx.core.graphics.ColorUtils.RGBToHSL;
import static androidx.core.graphics.ColorUtils.RGBToLAB;
import static androidx.core.graphics.ColorUtils.XYZToColor;
...
public class NotificationColorUtil {
    ...
    private static class ColorUtilsFromCompat {
         ...
         *
         *  <ul>  BT709   RGBToXYZ
         *   <li> [x     [0.4124   0.3576  0.1805       [R
         *   <li>  y  =   0.2126   0.7152  0.0722   *    G
         *   <li>  z]      0.0193  0.1192   0.9505]      B]
         * </ul>
         *
         ...
         *               b*=200[f(Y/Yn)-f(Z/Zn)]
         *               其中
         *               f(t)= {    t的1/3次方 ,如果t >(6/29)的三次方
         *               1/3*(29/6)的三次方 + 16/166 ,其他      }
         */
        public static void XYZToLAB(@FloatRange(from = 0f, to = ColorUtilsFromCompat.XYZ_WHITE_REFERENCE_X) double x,
                                    @FloatRange(from = 0f, to = ColorUtilsFromCompat.XYZ_WHITE_REFERENCE_Y) double y,
                                    @FloatRange(from = 0f, to = ColorUtilsFromCompat.XYZ_WHITE_REFERENCE_Z) double z,
                                    @NonNull double[] outLab) {
            if (outLab.length != 3) {
                throw new IllegalArgumentException("outLab must have a length of 3.");
            }

            x = pivotXyzComponent(x / ColorUtilsFromCompat.XYZ_WHITE_REFERENCE_X);
            y = pivotXyzComponent(y / ColorUtilsFromCompat.XYZ_WHITE_REFERENCE_Y);
            z = pivotXyzComponent(z / ColorUtilsFromCompat.XYZ_WHITE_REFERENCE_Z);
        ...
        private static double pivotXyzComponent(double component) {
            return component > ColorUtilsFromCompat.XYZ_EPSILON
                    ? Math.pow(component, 1 / 3.0)
                    : (ColorUtilsFromCompat.XYZ_KAPPA * component + 16) / 116;
        }
    ...
    public static int ensureLargeTextContrast(int color, int bg, boolean isBgDarker) {
        return isBgDarker ? findContrastColorAgainstDark(color, bg, true, 3)
     ...
     * the same hue as the original color. 为具有给定文本颜色和提示文本颜色的文本视图查找背景颜色，该颜色与原始颜色具有相同的色调。
     */
    ...
    private static String contrastChange(int colorOld,int colorNew,int bg){
        return String.format(Locale.ROOT,"from %.2f:1 to %.2f:1",
